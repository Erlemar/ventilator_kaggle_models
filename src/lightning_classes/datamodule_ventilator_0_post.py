...
class VentilatorDataModule(pl.LightningDataModule):
 ...
 'u_out_lagback_diff4'] + ['fold'] + ['id', 'breath_id', 'one', 'count', 'breath_id_lag', 'breath_id_lag2', 'breath_id_lagsame',
    ...
    def make_features20(self, data):
        data['time_step_diff'] = data.groupby('breath_id')['time_step'].diff().fillna(0)
        data['inhailed_air'] = data['time_step_diff'] * data['u_in']
        data['area'] = data.groupby('breath_id')['inhailed_air'].cumsum()
        data['log_u_in'] = np.log1p(data['u_in'])

        data['u_in_cumsum'] = (data['u_in']).groupby(data['breath_id']).cumsum()

        data['log_u_in_diff1'] = data.groupby('breath_id')['log_u_in'].diff(1)
        data['log_u_in_diff2'] = data.groupby('breath_id')['log_u_in'].diff(2)
        data['log_u_in_diff3'] = data.groupby('breath_id')['log_u_in'].diff(3)
        data['log_u_in_diff4'] = data.groupby('breath_id')['log_u_in'].diff(4)
    ...
    def setup(self, stage=None):
        ...
        if os.path.exists(os.path.join(self.cfg.datamodule.path, f'train_{self.cfg.datamodule.make_features_style}.csv')):
            ...
            train['pressure_diff1'] = train.groupby(['breath_id'])['pressure'].diff(1).fillna(0)
            train['pressure_diff2'] = train.groupby(['breath_id'])['pressure'].diff(2).fillna(0)
            ...
            train_targets1 = train.loc[train['fold'] != self.cfg.datamodule.fold_n, 'pressure_diff1'].copy().values.reshape(-1, 80)
            valid_targets1 = train.loc[train['fold'] == self.cfg.datamodule.fold_n, 'pressure_diff1'].copy().values.reshape(-1, 80)

            train_targets2 = train.loc[train['fold'] != self.cfg.datamodule.fold_n, 'pressure_diff2'].copy().values.reshape(-1, 80)
            valid_targets2 = train.loc[train['fold'] == self.cfg.datamodule.fold_n, 'pressure_diff2'].copy().values.reshape(-1, 80)
            ...
            test_targets1 = np.zeros(len(test)).reshape(-1, 80)
            test_targets2 = np.zeros(len(test)).reshape(-1, 80)
        else:
            ...
            elif self.cfg.datamodule.make_features_style == 20:
                train = self.make_features20(train)
                test = self.make_features20(test)
        ...
        for col in ['id', 'breath_id', 'one', 'count', 'breath_id_lag', 'breath_id_lag2', 'breath_id_lagsame',
                   'breath_id_lag2same', 'pressure', 'pressure_diff1', 'pressure_diff2']:
       ...
       '7_in_max', 'R_mult_c_250', 'RC_sum']
        ...
        self.train_dataset = dataset_class(train_data, mode='train', u_out=train_u_out_, pressure=train_targets, pressure1=train_targets1, pressure2=train_targets2)
        self.valid_dataset = dataset_class(valid_data, mode='valid', u_out=valid_u_out_, pressure=valid_targets, pressure1=valid_targets1, pressure2=valid_targets2)
        self.test_dataset = dataset_class(test, mode='test', u_out=test_u_out, pressure=test_targets, pressure1=test_targets1, pressure2=test_targets2)
