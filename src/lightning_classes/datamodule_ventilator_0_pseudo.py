 ...
 'u_out_lagback_diff4'] + ['fold'] + ['id', 'breath_id', 'one', 'count', 'breath_id_lag', 'breath_id_lag2', 'breath_id_lagsame',
    ...
    def setup(self, stage=None):
        ...
        if os.path.exists(os.path.join(self.cfg.datamodule.path, f'train_{self.cfg.datamodule.make_features_style}.csv')):
            ...
            if not os.path.exists(os.path.join(self.cfg.datamodule.path, f'pseudo_{self.cfg.datamodule.make_features_style}.csv')):
                if self.cfg.training.debug:
                    pseudo = pd.read_csv(os.path.join(self.cfg.datamodule.path, self.cfg.datamodule.pseudo_path),
                                         nrows=80000)
                    test1 = pd.read_csv(os.path.join(self.cfg.datamodule.path, 'test.csv'), nrows=80000)
                else:
                    pseudo = pd.read_csv(os.path.join(self.cfg.datamodule.path, self.cfg.datamodule.pseudo_path))
                    test1 = pd.read_csv(os.path.join(self.cfg.datamodule.path, 'test.csv'))
                pseudo_train = test1.copy()
                pseudo_train['pressure'] = pseudo['pressure'].copy()
                pseudo_train['fold'] = 1000
                del pseudo, test1
                pseudo_targets = pseudo_train['pressure'].copy().values.reshape(-1, 80)
                pseudo_u_out_ = pseudo_train['u_out'].copy().values.reshape(-1, 80)
                if self.cfg.datamodule.make_features_style == 0:
                    pseudo_train = self.make_features(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 1:
                    pseudo_train = self.make_features1(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 2:
                    pseudo_train = self.make_features2(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 3:
                    pseudo_train = self.make_features3(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 4:
                    pseudo_train = self.make_features4(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 5:
                    pseudo_train = self.make_features5(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 6:
                    pseudo_train = self.make_features6(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 7:
                    pseudo_train = self.make_features7(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 8:
                    pseudo_train = self.make_features8(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 9:
                    pseudo_train = self.make_features9(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 10:
                    pseudo_train = self.make_features10(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 11:
                    pseudo_train = self.make_features11(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 12:
                    pseudo_train = self.make_features12(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 13:
                    pseudo_train = self.make_features13(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 14:
                    pseudo_train = self.make_features14(pseudo_train)
                elif self.cfg.datamodule.make_features_style == 15:
                    pseudo_train = self.make_features15(pseudo_train)

                pseudo_train.to_csv(
                    os.path.join(self.cfg.datamodule.path, f'pseudo_{self.cfg.datamodule.make_features_style}.csv'),
                    index=False)


            else:
                pseudo_train = pd.read_csv(os.path.join(self.cfg.datamodule.path, f'pseudo_{self.cfg.datamodule.make_features_style}.csv'))
                pseudo_train['fold'] = 1000
            ...
            if self.cfg.training.debug:
                ...
                pseudo = pd.read_csv(os.path.join(self.cfg.datamodule.path, self.cfg.datamodule.pseudo_path), nrows=80000)
            else:
            ...
            pseudo_train.to_csv(os.path.join(self.cfg.datamodule.path, f'pseudo_{self.cfg.datamodule.make_features_style}.csv'), index=False)
        ...
        for col in ['id', 'breath_id', 'one', 'count', 'breath_id_lag', 'breath_id_lag2', 'breath_id_lagsame',
            ...
            if col in pseudo_train.columns:
                pseudo_train.drop(col, axis=1, inplace=True)
       ...
       '7_in_max', 'R_mult_c_250', 'RC_sum']
        ...
        if self.cfg.datamodule.normalize:
            if not self.cfg.datamodule.normalize_all:
                ...
                pseudo_data = RS.transform(pseudo_train.drop(['fold'], axis=1))
        ...
        train_data = np.vstack([train_data, pseudo_data])
        train_u_out_ = np.vstack([train_u_out_, pseudo_u_out_])
        train_targets = np.vstack([train_targets, pseudo_targets])
