...
class VentilatorDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if self.cfg.dataing.debug:
            data = pd.read_csv(os.path.join(self.cfg.datamodule.path, 'data.csv'), nrows=196000)
            test = pd.read_csv(os.path.join(self.cfg.datamodule.path, 'test.csv'), nrows=80000)
        else:
            data = pd.read_csv(os.path.join(self.cfg.datamodule.path, 'data.csv'))
        ...
        data = data.reshape(-1, 80, data.shape[-1])
        test = test.reshape(-1, 80, data.shape[-1])
        gkf = KFold(n_splits=self.cfg.datamodule.n_folds, shuffle=True, random_state=self.cfg.dataing.seed)
        ...
        #     gkf = GroupShuffleSplit(n_splits=self.cfg.datamodule.n_folds, random_state=self.cfg.dataing.seed)

        splits = list(gkf.split(X=data, y=targets))
        data_idx, valid_idx = splits[self.cfg.datamodule.fold_n]

        data_data = data[data_idx].copy()
        valid_data = data[valid_idx].copy()

        data_u_out_ = data_u_out[data_idx].copy()
        valid_u_out_ = data_u_out[valid_idx].copy()

        data_targets = targets[data_idx].copy()
        ...
        self.data_dataset = dataset_class(data_data, mode='data', u_out=data_u_out_, pressure=data_targets)
